package com.CPGroupH.error.handler;
...
import com.CPGroupH.response.ValidationError;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     * <p>
     * ex) @Valid가 붙은 객체가 컨트롤러 메서드의 파라미터로 전달되었을 때, 해당 객체의 필수 필드가 누락되거나 잘못된 형식의 데이터가 전달되면 발생한다.
     */
    @Override
     ...
     * <p>
     * ex) @PathVariable Long id가 있는데 요청 URL에서 /api/items/abc (abc는 Long으로 변환될 수 없음) 처럼 요청한 경우, 즉, 기대하는 타입과 다른 타입이 전달된
     * 경우 발생한다.
     *
    ...
    public ResponseEntity<Object> handleMethodArgumentTypeMismatch(MethodArgumentTypeMismatchException ex) {
        ErrorCode errorCode = CommonErrorCode.BAD_REQUEST;
        String errorMessage = String.format("Parameter '%s' should be of type '%s'", ex.getName(), ex.getRequiredType()
                .getSimpleName());
     ...
     * <p>
     * ex) 존재하지 않는 경로로 요청이 들어오는 등, 잘못된 URL로 요청이 들어왔을 때 발생한다.
     *
